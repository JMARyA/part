...
pub fn split_file(f: &str, options: SplitOptions) {
    ...
    match options {
        SplitOptions::NumberOfParts(n) => {
            let nsize = fsize / n;
            println!("Splitting file in {n} parts with size {nsize}B");
            make_split_files(&mut file, content, n, nsize, f);
        }
        SplitOptions::SizeOfParts(s) => {
            let n = fsize / s;
            println!("Splitting file in {n} parts with size {s}B");
...
fn make_split_files(file: &mut File, content: Vec<u8>, n: usize, s: usize, f: &str) {
    ...
    for i in 0..n {
        ...
        let mut read_buf: Vec<u8> = vec![0u8; s];
        ...
        // WRITE FILE
        part.write_all(&read_buf).unwrap();
    }

    let mut info_f = File::create(format!("{}.partinfo", &f)).unwrap();
    let info_s = serde_json::to_vec(&info).unwrap();
    info_f.write_all(&info_s).unwrap();
}
...
pub fn combine_file(f: &str) {
    ...
    for i in 0..info.number_of_parts {
        let mut read = vec![];
        let mut part_f = File::open(format!("{basename}.{i}.part")).unwrap();
    ...
    println!("File successfully combined");
    let mut ff = File::create(basename).unwrap();
    ff.write_all(&final_content).unwrap();
}
